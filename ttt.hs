...
availableMoves :: Field -> Set.Set Int
availableMoves field = Set.fromList [1,2,3,4,5,6,7,8,9] \\ (crosses field) \\ (noughts field)

move :: Field -> Int -> Cell -> Field    
move field n Cross = field { crosses = Set.insert n (crosses field) }
move field n Nought = field { noughts = Set.insert n (noughts field) }

initField = Field { crosses = Set.empty, noughts = Set.empty }
...
minimax field movesNext =
    ...
    Just Cross -> (1 + length (availableMoves field), Nothing)
    Just Nought -> (-1 - length (availableMoves field), Nothing)
    _ -> if movesNext == Cross
             then foldr maximize (minBound :: Int, Nothing) $ availableMoves field
             else foldr minimize (maxBound :: Int, Nothing) $ availableMoves field 
                where
